...
## Status
ACTIVE — v3.1.2 is published from merge/tag commit `1c25c06` with Windows MSI/EXE, macOS universal PKG, and three Linux GUI packages. Hosted Windows qualification passed MSI/EXE construction, associated-icon extraction, synthetic-prior same-channel update with exact icon hashes, immutable v2 migrations, uninstall cleanup, public attachment, and post-public verification. The public Windows GUI ZIP sidecar and all five manifest-owned icon files were independently rechecked. The only remaining acceptance gate is the operator-visible Windows notification-area confirmation that the tray glyph is non-generic and its hover/Quit behavior is correct.
