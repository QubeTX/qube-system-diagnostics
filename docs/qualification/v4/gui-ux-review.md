...
That pass exposed the page omission, unsupported arrow glyphs and interface priority issue above. The final source changes pass 63 ordinary native tests, with two opt-in benchmarks skipped, and a freshly generated strict view/model contract. Layout fixtures cover all ten destinations in both modes at 960, 1180 and 1600 pixels; they establish control bounds, not readable text or screen-reader support. A release-style rebuild and final live readback of those follow-up changes are in progress.
