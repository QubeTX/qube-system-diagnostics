...
- Packaging:
  ...
  - Windows and macOS qualification compile a synthetic prior-version binary and prove real same-channel replacement, rollback cleanup, complete proven-owner CLI uninstall, and final version verification rather than only checking an already-current install. Windows additionally proves `sd300 install` transfers all four native channels to the preferred managed PowerShell owner.
