...
- [x] Native strict checks, GUI tests, Rust tests, Windows release build, distribution-lock validation, archive validation, and WiX builds pass.
- [x] The transparent follow-up passes deterministic/semantic checks at every generated size; the rebuilt Windows EXE exposes the mark through associated-icon extraction and its exact archive manifest owns and verifies the replacement PNG/ICO.
...
## Status
ACTIVE — PUBLIC in v3.1.3 from immutable commit `d489654`. PR #9 passed every core, security, cargo-dist, and six-target native GUI lane before merge. Release 30150345302, Windows producer 30150538506, macOS producer 30150538503, Linux producer 30150538512, and Qualify/Publish 30151050783 all passed; the repaired Mac oracle built, signed, notarized, stapled, installed, updated, and uninstalled the universal package on Intel and Apple Silicon. Public verification confirms the tag/latest release/crate all resolve to 3.1.3 at `d489654`, all 59 assets are present, the downloaded Windows/macOS archives match their sidecars and SLSA provenance, every packaged file matches its install manifest, the public EXE exposes the selected associated icon, and an isolated crates.io install reports `sd300 3.1.3`. The only remaining acceptance item is the operator-visible Windows taskbar/Alt+Tab/notification-area appearance check; automation cannot inspect the Windows tray.

## Activity
- 2026-07-25 03:40 — PR #9 CI run 30150105978 passed every core/security/cargo-dist check plus Windows, Intel/Apple Silicon macOS, GNU x86-64/ARM64 Linux, and musl Linux native GUI targets; merged as `d489654`, then Release 30150345302 rebuilt the exact six-target draft (agent: codex)
- 2026-07-25 03:40 — Windows producer 30150538506 passed GUI/icon packaging, MSI/EXE, fresh/synthetic/immutable-prior update routes, ownership hashes, and uninstall; macOS 30150538503 passed the repaired `app-icon.icns` package oracle plus signing/notarization and both native lifecycle lanes; Linux 30150538512 passed and attached all three packages (agent: codex)
- 2026-07-25 03:40 — Qualify/Publish 30151050783 attempt 1 stopped pre-publication on an isolated GitHub transparency-log 404 after all artifact/lifecycle gates passed; one bounded unchanged-artifact rerun cleared all three attestations, published v3.1.3/latest/crates.io, and passed the post-public Cargo and managed Linux lifecycle (agent: codex)
- 2026-07-25 03:40 — independently verified the public `v3.1.3` tag and 59-asset release at `d489654`, both downloaded GUI sidecars and provenance attestations, exact 10-file Windows and 42-file macOS install manifests, Win32 associated-icon extraction, Mac `app-icon.icns`/tray-template delivery, and isolated `cargo install tr300-tui --version 3.1.3` → `sd300 3.1.3`; manual Windows tray/taskbar appearance remains operator-owned (agent: codex)
- 2026-07-25 02:52 — reproduced Native SDK's exact macOS package contract locally: the app bundle contains `Contents/Resources/app-icon.icns`, `Info.plist` declares that same basename, and no legacy `AppIcon.icns` exists; the focused validator fix also passes Git Bash syntax, strict Native model/manifest checking, Rust formatting, and all 16 coordinated v3.1.3 version surfaces (agent: codex)
- 2026-07-25 02:43 — merged PR #8 to `main` as `f2a49bb`; Release 30149273467 passed and created the exact v3.1.3 draft, Linux producer 30149456761 passed/attached all packages, and both architecture-native Mac builds passed, but macOS producer 30149456797 failed because final validation expected `AppIcon.icns` while the explicit prebuilt container correctly shipped as `app-icon.icns` (agent: codex)
- 2026-07-25 02:43 — verified no v3.1.3 tag or public release existed, canceled the unfinished Windows producer, deleted only the unpublished draft, confirmed no active main workflows remained, and opened `codex/sd300-macos-icon-package-fix` for reviewed recovery without changing the version (agent: codex)
- 2026-07-25 02:18 — initial PR #8 matrix run 30148356302 passed every job, including clean hosted Windows, Intel/Apple Silicon macOS, GNU x86-64/ARM64 Linux, and musl Linux native GUI packages; operator then explicitly authorized a full public update (agent: codex)
- 2026-07-25 02:18 — coordinated the release candidate to v3.1.3 across all 16 checked surfaces; local format/clippy, 121 root tests, 10 engine tests, 37/39 Native tests with two expected skips, release build, crate publish dry-run, Windows target-pinned build, distribution/path-leak checks, and v3.1.3 updater ZIP packaging passed (agent: codex)
- 2026-07-25 02:00 — launched only the staged Windows candidate with an isolated settings root; Computer Use confirmed the new cube in native title chrome, then closed the candidate cleanly with no public SD-300 process or preferences touched. Taskbar/tray physical appearance remains operator-owned (agent: codex)
- 2026-07-25 01:54 — implemented separate production SVG masters for transparent Windows/Linux and a softened transparent-corner macOS halo; regenerated ICO/ICNS/runtime/hicolor assets, retained the original selected raster as evidence, and left all tray derivatives unchanged (agent: codex)
- 2026-07-25 01:54 — passed deterministic/semantic icon checks at every size, Native tests (37/39 with two expected skips), 121 shared-app tests, 10 engine tests, a release-shaped Windows build, distribution/path-leak checks, Win32 associated-icon extraction, and exact 10-file archive manifest/hash verification; the candidate carries the replacement PNG/ICO under the existing updater/uninstall ownership paths (agent: codex)
- 2026-07-25 01:28 — operator approved the transparent app-icon preview and requested implementation across supported GUI platforms, explicitly allowing a creative softened transparent macOS treatment; created `codex/sd300-transparent-app-icon`, added the required board subtask, and preserved the existing monochrome tray identity (agent: codex)
...
- 2026-07-23 05:02 — merged PR #7 as `1c25c06`; Release 29995616736 and all platform producers passed. Windows 29996060863 proved MSI/EXE delivery, associated icon, synthetic-prior managed update with owned icon hashes, conservative uninstall cleanup, and immutable v2 transitions; macOS 29996060870 and Linux 29996060878 built, installed/qualified, and attached their platform packages (agent: codex)
- 2026-07-23 05:02 — Qualify/Publish 29997304182 passed all 19 steps and published non-yanked `tr300-tui`/GitHub Release v3.1.2 from `1c25c06`; final public Windows ZIP audit matched sidecar SHA-256 `29ee3a337d510ca30b1686ccede69b592c7233ba67152adc2a1bca3dbda9f9b9` and confirmed manifest-owned app-icon.png/app-icon.ico/tray-icon.ico/tray-icon-dark.ico/tray-icon-light.ico bytes (agent: codex)
