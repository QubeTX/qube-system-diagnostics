...
## Status
ACTIVE — the approved transparency follow-up is implemented on `codex/sd300-transparent-app-icon`: Windows/Linux use a free-floating transparent orange/off-white cube, macOS uses a transparent-corner isometric graphite halo, and the selected monochrome tray remains byte-for-byte unchanged. Deterministic export, every small-size semantic guard, Native/Rust/engine tests, the release-shaped Windows build, Win32 associated-icon extraction, distribution locks, and exact candidate-archive manifest hashes pass locally. The public v3.1.2 installation was not replaced. Hosted macOS/Linux packages and the historical operator-visible Windows notification-area confirmation remain release/physical gates.
