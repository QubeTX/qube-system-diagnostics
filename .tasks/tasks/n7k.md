...
## Status
ACTIVE — PR #8 carries the approved transparency follow-up and is now a coordinated v3.1.3 release candidate by explicit operator authorization. Windows/Linux use a free-floating transparent orange/off-white cube, macOS uses a transparent-corner isometric graphite halo, and the selected monochrome tray remains byte-for-byte unchanged. The initial all-platform PR matrix passed every core/security/cargo-dist job and all six native GUI targets. The bumped v3.1.3 candidate passes all 16 version surfaces, Native/Rust/engine tests, release build, publish dry-run, distribution/path-leak locks, Windows associated-icon extraction, and exact updater-archive manifest hashes locally. It still requires the final bumped PR matrix and complete main-branch release/producer/qualify chain. The public v3.1.2 installation remains untouched.
