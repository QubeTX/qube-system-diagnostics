...
## Status
ACTIVE — PR #8 merged the approved, fully green v3.1.3 candidate to `main` at `f2a49bb`. The first Release workflow built every immutable artifact and created the correctly targeted unpublished draft; Linux production packages passed and attached, but final macOS PKG validation exposed a filename-contract mismatch after both architecture builds passed: Native SDK preserved the explicit `app-icon.icns` basename while the release script still required the old generated `AppIcon.icns` name. No tag, crate, or public release existed. The Windows producer was canceled, the unpublished draft was deleted, and all main workflows stopped before recovery. `codex/sd300-macos-icon-package-fix` now aligns validation with the prebuilt-container contract and must pass a fresh reviewed candidate before 3.1.3 restarts. The public v3.1.2 installation remains untouched.
