...
ACTIVE, owner Codex. The updater/discovery corrections are implemented in PR #11 at efbab9e. Windows composite lifecycle and native shell discovery pass; final Intel/musl GUI input timing exceeds the accepted 100 ms gate. Await the owner's specific timing-deferral decision or a separate GUI latency correction; no further blind repeat is scheduled. The actual public release/local installation remain 4.0.0, and website PR #18 remains held. Evidence-only follow-up is on codex/sd300-401-qualification-evidence so saving results does not restart unchanged GUI qualification.
